    ...
    if isinstance(output, list):
        ...
        for item in output:
            if isinstance(item, str):
                ...
                url = item.get("url") or item.get("file") or item.get("video")
                if isinstance(url, str):
                    urls.append(url)
        return urls
    if isinstance(output, dict):
        url = output.get("url") or output.get("file") or output.get("video")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a single video with Replicate.")
    ...
    parser.add_argument("--image", type=Path, help="Optional first-frame or reference image for image-to-video.")
    parser.add_argument("--outdir", type=Path, default=Path("studio/outputs/replicate/video"))
    parser.add_argument("--basename", default="chloe-replicate-video-test")
    parser.add_argument("--model", default="bytedance/seedance-2.0-fast")
    ...
    if args.image:
        ...
        model_input["image"] = file_data_url(args.image)
    for key, value in args.input:
        model_input[key] = value

    args.outdir.mkdir(parents=True, exist_ok=True)
    video_path = args.outdir / f"{args.basename}.{args.extension}"
